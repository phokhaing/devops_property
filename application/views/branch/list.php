...
<div class="white-area-content">
    <div class="db-header clearfix">
        <div class="page-header-title"> <span class="glyphicon glyphicon-send"></span> <?php echo lang("branch") ?>
        </div>
        <div class="db-header-extra form-inline">
            ...
            <?php if($this->authorization->hasPermission($moduleName, "create")): ?>
                <a href="<?php echo site_url("branch/add") ?>" class="btn btn-primary btn-sm"><?php echo lang("add_branch") ?></a>
            <?php endif; ?>
        ...
        <table id="table" class="table table-bordered table-hover table-striped">
            <thead>
                <tr class="table-header">
                        ...
                        <th><?php echo lang('branch_name_kh'); ?></th>
            ...
            <tbody id="list-branch" class="small-text">
                    <?php 
                        $i=1;
                        foreach ($data as $row): ?>
                        <tr>
                            <td><b><?php echo $i++; ?></b></td>
                                ...
                                <td><b><?php echo ucwords($row['branch_name_kh']); ?></b></td>
                                ...
                                <td class="center-table-data">
                                    ...
                                    <?php if($this->authorization->hasPermission($moduleName, "view")):?>
                                        <a href="<?php echo base_url('branch/view?id=').$row['id_branch']; ?>" class="btn btn-info btn-sm" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="View">View</a> 
                                    <?php endif; ?>

                                    <?php if($this->authorization->hasPermission($moduleName, "update")):?>
                                        <a href="<?php echo base_url('branch/edit?id=').$row['id_branch']; ?>" class="btn btn-warning btn-sm" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Edit"><span class="glyphicon glyphicon-cog"></span></a> 
                                    <?php endif; ?>

                                    <?php if($this->authorization->hasPermission($moduleName, "delete")):?>
                                        <a href="<?php echo base_url('branch/delete?id=').$row['id_branch']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this branch?')" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Delete"><span class="glyphicon glyphicon-trash"></span></a>
                                    <?php endif; ?>
                                </td>
                        </tr>
                    <?php endforeach; ?>
...
<script type="text/javascript">
    ...
    var table;
    var baseURL = "<?php echo site_url(); ?>branch";
    ...
    function filter(){
        ...
        $.ajax({
            url: global_base_url + "branch/findFilter/" + value,
            type: "get",
            success: function (response) {
                $('#list-branch').html(response);
            }
        });
    }
