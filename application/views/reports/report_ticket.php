...
<div class="white-area-content">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenu1">
                        ...
                        <li>
                            ...
                            <?php foreach ($categories->result() as $r) : ?>
                                <a href="<?php echo site_url("reports/" . $page . "/".$r->ID.$status_filter.$date_filter) ?>"><?php echo $r->name ?></a>
                                    <?php if(!empty(getSubCategoryByParent($r->ID))): ?>
                                        <?php foreach (getSubCategoryByParent($r->ID) as $sub):?>
                                          <ul style='list-style:none;' class='sub_links'>
                                            <li>
                                              <a href="<?php echo site_url("reports/" . $page . "/" . $sub->ID.$status_filter.$date_filter) ?>"><?php echo $sub->name ?></a>
                                            </li>
                                          </ul>
                                       <?php endforeach; ?>
                                    <?php endif; ?>
    ...
    <?php if ($mode == 0) : ?>
            <?php 
                $filter = '';
                if(isset($_GET['progress']) && isset($_GET['close'])){
                $filter = '&progress='.$_GET['progress'].'&close='.$_GET['close'];
            } ?>
            <a href="<?php echo site_url("reports/" . $page . "/" . $catid . "?mode=1".$filter) ?>" class="btn btn-default"><?php echo lang("ctn_855") ?></a>
        <?php endif; ?>
...
<script type="text/javascript">
    ...
    $('#status-progress').click(function(){
        ...
        window.location.href = "<?php echo base_url().'reports/'.$page.'/'.$catid.'?progress='?>"+progress+'&close='+close+s_date+e_date;
