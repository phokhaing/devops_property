 ...
 *------------------------------------------------------
 */
class FloatAdvanceModel extends CI_Model
{	
	/* table */
	public $table       = "float_advance";	
	public $tblItems    = "float_advance_items";
	public $tblFiles    = "float_advance_files";
	private $tblHistory = "float_advance_history";
	...
	/* items */
	private $float_advance_id = 'float_advance_id';
    ...
    private $hook_request   = 'request_float_advance';
    private $hook_check   	= 'check_float_advance';
    private $hook_approve   = 'approve_float_advance';
	private $hook_reject    = 'reject_float_advance';
	...
	{	
		...
		if($id){
             ...
             */
            $primary_key = $id;
            $title = 'has requested a float advance form and transfers for you to check.';
      ...
      */
   	...
   	{   
        ...
        if($status == 'approved'){
	        ...
	        $title = 'has approved your float advance form.';
	        $email_template = getEmailTempalteByHook($this->hook_approve, $this->email_lang);
	    }elseif($status == 'checked'){
	        // set email template transfer
	        $title = 'has checked a float advance form and transfers for you to approve.';
	        ...
	        $title = 'has rejected your float advance form.';
	...
	function findLastID(){
		...
		if(!empty($output)){
			...
			return 'ADV-'.date('y-m-').$digit;
		}
		return 'ADV-'.date('y-m-').'0001';
	...
	public function findFiles($id){
		return $this->db
					->where($this->float_advance_id, $id)
