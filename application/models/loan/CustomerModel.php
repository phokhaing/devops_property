...
{	
	/* table */
	private  $table       			  = "loan_customer";	
	private $tblLoanCustomerHistory   = "loan_customer_history";	
	private $tblSector  			  = "loan_sector";
	private $tblIndustry  			  = "loan_industry";
	private $tblIdentificationType 	  = "loan_identification_type";
	private $tblBusinessType  	      = "loan_business_type";
	private $tblBusinessLocationStatus= "loan_business_location_status";
	private $tblBusinessFormat  	  = "loan_business_format";
	private $tblBusinessStatus  	  = "loan_business_status";
	private $tblIdentificationFiles   = "loan_identification_files";
	private $tblAttachmentFiles       = "loan_attachment_files";
	...
	/* contact */
	private $tblContact		= "loan_customer_contact";
	...
	private $tblIdentification 			= 'loan_identification';
	...
	/* employment detail */
	private $tblEmploymentDetail	= "loan_employment_detail";
	...
	/* business plan */
	private $tblBusinessPlan		= "loan_business_plan";
	...
	private $tblBusinessIncomeExpend = "loan_business_income_expend";
	private $business_income_id 	= "business_income_id";
	private $income_date 		    = "income_date";
	private $business_currency 		= "business_currency";
	...
	private $tblPersonalIncomeExpend   = "loan_personal_income_expend";
	private $personal_income_id 	   = "personal_income_id";
	private $personal_currency 		   = "personal_currency";
	...
	/** 
      ...
      * @author: khaing.pho1991@gmail.com
      * @param: 05/July/2020
      * @param: method for check identification code existed or not
      *----------------------------------------------------------------
      */
	public function is_existed(){
		for($i=0; $i < count($this->input->post($this->identification_code)); $i++)
		{
			$id_code = $this->db->where($this->identification_code, $this->input->post($this->identification_code)[$i])
				->limit(1)
				->get($this->tblIdentification)
				->row();
			if($id_code){
				return true;
				break;
	...
	function create()
	{
		...
		if($customer_id){
			/**
			 * ------------------------------------------
			 * CREATE CUSTOMER CODE (CU (10LENGTH) 
			 * ------------------------------------------
			 */
			$customer_code = 'CU'.str_pad($customer_id, 10, "0", STR_PAD_LEFT);
			$this->db->where($this->id, $customer_id)
					 ->set($this->customer_id, $customer_code)
					 ->update($this->table);
			...
			/**
			 * ------------------------------------------
			 * CREATE NEW EMPLOYMENT DETAIL INFO
			 * ------------------------------------------
			 */
			 ...
			 * ------------------------------------------
			 */
			if($this->input->post($this->business_name)[0] && $this->input->post($this->business_type)[0])
			{
				for($i=0; $i < count($this->input->post($this->business_name)); $i++){ 
					...
					if($business_plan_id){
						...
						$this->business_currency    => $this->input->post($this->business_currency)[$i],
			...
			/**
			 * ------------------------------------------
			 * CREATE NEW PERSONAL INCOME AND EXPEND INFO
			 * ------------------------------------------
			 */
			$this->db->insert($this->tblPersonalIncomeExpend, array(
				$this->customer_id 	  		     => $customer_id,
				$this->personal_currency 		 => $this->input->post($this->personal_currency),
			 ...
			 * ------------------------------------------
			 */
	        $filePath    = '/loan_files/customers/'.$customer_code.'/identification';
			 ...
			 * ------------------------------------------
			 */
	        $filePath    = '/loan_files/customers/'.$customer_code.'/attachment';
	        ...
	        if(!empty($fileUpload)){
	            foreach ($fileUpload as $file) {
	                $this->db->insert($this->tblAttachmentFiles, array(
	                "customer_id"     => $customer_id,
	                "upload_file_name"=> $file['upload_file_name'],
	                "original_name"   => $file['original_name'],
	                "file_type"       => $file['file_type'],
	...
	{	
		...
		$this->db->where($this->id, $customer_id)->update($this->table, array(
			$this->firstname_kh   => $this->input->post($this->firstname_kh),
			$this->lastname_kh	  => $this->input->post($this->lastname_kh),
			$this->firstname_en   => $this->input->post($this->firstname_en),
			$this->lastname_en	  => $this->input->post($this->lastname_en),
		 ...
		 * ------------------------------------------
		 */
		if($this->input->post($this->spouse_firstname_kh))
		{	
			if($this->input->post($this->spouse_id)){
				/* update spouse if isset spouse_id */
				$this->db->where($this->spouse_id, $this->input->post($this->spouse_id))->update($this->tblSpouse, array(
				$this->spouse_firstname_kh => $this->input->post($this->spouse_firstname_kh),
				$this->spouse_lastname_kh  => $this->input->post($this->spouse_lastname_kh),
				$this->spouse_firstname_en => $this->input->post($this->spouse_firstname_en),
				$this->spouse_lastname_en  => $this->input->post($this->spouse_lastname_en),
				...
				/* create new spouse if not isset spouse_id */
				$this->db->insert($this->tblSpouse, array(
				$this->customer_id 		   => $customer_id,
				$this->spouse_firstname_kh => $this->input->post($this->spouse_firstname_kh),
				$this->spouse_lastname_kh  => $this->input->post($this->spouse_lastname_kh),
		...
		}else if($this->input->post($this->spouse_id)){
			/* delete spouse if isset spouse_id and not isset spouse form */
			$this->db->where($this->spouse_id, $this->input->post($this->spouse_id))->delete($this->tblSpouse);
		}

		
		/**
		 * ------------------------------------------
		 * UPDATE CONTACT ADDRESS INFO BY CONTACT ID
		 * ------------------------------------------
		 */
		/* delete contact that deleted at form */
		if($this->input->post('contact_deleted')){
			$contact_deleted = explode('___', $this->input->post('contact_deleted'));
			foreach ($contact_deleted as $key => $con_id) {
				if($con_id != null){
					$this->db->where($this->contact_id,$con_id)->delete($this->tblContact);
				}
			}
		}
		for($i=0; $i < count($this->input->post($this->country_id)); $i++)
		{ 
			if($this->input->post($this->contact_id)[$i]){
				/* update contact if isset contact_id */
				$this->db->where($this->contact_id, 
				$this->input->post($this->contact_id)[$i])->update($this->tblContact, array(
				$this->country_id 	  => $this->input->post($this->country_id)[$i],
				$this->province_id    => $this->input->post($this->province_id)[$i],
				$this->district_id    => $this->input->post($this->district_id)[$i],
				$this->commune_id 	  => $this->input->post($this->commune_id)[$i],
				...
				/* create new spouse if not isset contact_id */
				$this->db->insert($this->tblContact, array(
				$this->customer_id 	  => $customer_id,
				$this->country_id 	  => $this->input->post($this->country_id)[$i],
				$this->province_id    => $this->input->post($this->province_id)[$i],
		...
		/**
		 * ------------------------------------------
		 * UPDATE IDENTIFICATION INFO BY ID
		 * ------------------------------------------
		 */
		/* delete identification that deleted at form */
		if($this->input->post('identification_deleted')){
			$identification_deleted = explode('___', $this->input->post('identification_deleted'));
			foreach ($identification_deleted as $key => $iden_id) {
				if($iden_id != null){
					$this->db->where($this->identification_id,$iden_id)->delete($this->tblIdentification);
				}
			}
		}
		for($i=0; $i < count($this->input->post($this->identification_type)); $i++)
		{ 
			if($this->input->post($this->identification_id)[$i]){
				/* update identification if isset identification_id */
				$this->db->where($this->identification_id, 
				$this->input->post($this->identification_id)[$i])->update($this->tblIdentification, array(
				$this->identification_type 	   	  => $this->input->post($this->identification_type)[$i],
				$this->identification_code 	      => $this->input->post($this->identification_code)[$i],
				$this->identification_issue_place => $this->input->post($this->identification_issue_place)[$i],
				$this->identification_issue_date  => date('Y-m-d', strtotime($this->input->post($this->identification_issue_date)[$i])),
				...
				/* create new identification if not isset identification_id */
				$this->db->insert($this->tblIdentification, array(
				$this->customer_id 	  			  => $customer_id,
				$this->identification_type 	   	  => $this->input->post($this->identification_type)[$i],
				$this->identification_code 	      => $this->input->post($this->identification_code)[$i],
		 ...
		 * ------------------------------------------
		 */
		/* delete employment that deleted at form */
		if($this->input->post('employment_deleted')){
			$employment_deleted = explode('___', $this->input->post('employment_deleted'));
			foreach ($employment_deleted as $key => $emp_id) {
				if($emp_id != null){
					$this->db->where($this->employment_id,$emp_id)->delete($this->tblEmploymentDetail);
				}
			}
		}
		for($i=0; $i < count($this->input->post($this->employment_type)); $i++)
		{ 
			if($this->input->post($this->employment_id)[$i]){
				/* update employment if isset employment_id */
				$this->db->where($this->employment_id, 
				$this->input->post($this->employment_id)[$i])->update($this->tblEmploymentDetail, array(
				$this->employment_type 		 => $this->input->post($this->employment_type)[$i],
				$this->self_employee 		 => $this->input->post($this->self_employee)[$i],
				$this->company_name 		 => $this->input->post($this->company_name)[$i],
				$this->empbusiness_type_id	 => $this->input->post($this->empbusiness_type_id)[$i],
				...
				/* create new employment detail if not isset identification_id */
				$this->db->insert($this->tblEmploymentDetail, array(
				$this->customer_id 	  		 => $customer_id,
				$this->employment_type 		 => $this->input->post($this->employment_type)[$i],
				$this->self_employee 		 => $this->input->post($this->self_employee)[$i],
		...
		/**
		 * ---------------------------------------------
		 * UPDATE BUSINESS INFO | BUSINESS INCOME EXPEND
		 * ---------------------------------------------
		 */
		/* delete business plan and business income expend that deleted at form */
		if($this->input->post('business_deleted'))
		{
			$business_deleted = explode('___', $this->input->post('business_deleted'));
			$business_inex_deleted = explode('___', $this->input->post('business_inex_deleted'));
			foreach ($business_deleted as $key => $bus_id) {
				if($bus_id != null){
					$this->db->where($this->business_plan_id,$bus_id)->delete($this->tblBusinessPlan);
					$this->db->where($this->business_income_id,$business_inex_deleted[$key])->delete($this->tblBusinessIncomeExpend);
				}
			}
		}
		/* if self_employee = Y, so must have at least a business plan form */
		if(in_array("Y", $this->input->post($this->self_employee)) && $this->input->post($this->business_name)[0] && $this->input->post($this->business_type)[0])
		{
			for($i=0; $i < count($this->input->post($this->business_name)); $i++){ 
				if($this->input->post($this->business_plan_id)[$i]){
					/* update business plan if isset business_plan_id */
					$this->db->where($this->business_plan_id, 
					$this->input->post($this->business_plan_id)[$i])->update($this->tblBusinessPlan, array(
					$this->business_name 	   => $this->input->post($this->business_name)[$i],
					$this->business_type 	   => $this->input->post($this->business_type)[$i],
					$this->business_status 	   => $this->input->post($this->business_status)[$i],
					$this->business_format 	   => $this->input->post($this->business_format)[$i],
					...
					/* update business income expend if isset business_plan_id */
					$this->db->where($this->business_income_id, 
					$this->input->post($this->business_income_id)[$i])->update($this->tblBusinessIncomeExpend, array(
					$this->income_date 		    => date('Y-m-d', strtotime($this->input->post($this->income_date)[$i])),
					$this->business_currency 	=> $this->input->post($this->business_currency)[$i],
					$this->business_profit 		=> $this->input->post($this->business_profit)[$i],
					$this->transport_cost 		=> $this->input->post($this->transport_cost)[$i],
					...
					/* create new business plan if not isset business_plan_id */
					$this->db->insert($this->tblBusinessPlan, array(
					$this->customer_id 	  	   => $customer_id,
					$this->business_name 	   => $this->input->post($this->business_name)[$i],
					$this->business_type 	   => $this->input->post($this->business_type)[$i],
					...
					/* create new business income expend if not exist busines plan id */
					if($business_plan_id){
						$this->db->insert($this->tblBusinessIncomeExpend, array(
						$this->business_plan_id 	=> $business_plan_id,
						$this->customer_id 	  	   => $customer_id,
		...
		}else if($this->input->post('businessPlanId')){
			/* delete business plan if isset businessPlanId but not isset business form */
			$businessPlanId = $this->input->post('businessPlanId');
			$businessInExId = $this->input->post('businessIncomeId');
			for($i=0; $i < count($businessPlanId); $i++){
				$this->db->where($this->business_plan_id,$businessPlanId[$i])->delete($this->tblBusinessPlan);
				$this->db->where($this->business_income_id,$businessInExId[$i])->delete($this->tblBusinessIncomeExpend);
			}
		}

		/**
		 * --------------------------------------------
		 * UPDATE PERSONAL INCOME AND EXPEND INFO BY ID
		 * --------------------------------------------
		 */
		$this->db->where($this->personal_income_id, 
		$this->input->post($this->personal_income_id))->update($this->tblPersonalIncomeExpend, array(
		$this->personal_currency 		 => $this->input->post($this->personal_currency),
		$this->personal_profit 		     => $this->input->post($this->personal_profit),
		$this->personal_income_salary    => $this->input->post($this->personal_income_salary),
		$this->personal_other_profit 	 => $this->input->post($this->personal_other_profit),
		 ...
		 * ------------------------------------------
		 */
		/* delete identification files that removed at form */
		if($this->input->post('identification_file_deleted')){
			$identification_file_deleted = explode('___', $this->input->post('identification_file_deleted'));
			$identification_file_path = explode('___', $this->input->post('identification_file_path'));
			foreach ($identification_file_deleted as $key => $fileid) {
				if($fileid != null){
					$this->db->where($this->id,$fileid)->delete($this->tblIdentificationFiles);
					unlink('./'.$identification_file_path[$key]);  
				}
			}
		}
		/* add more identification files */
        $filePath    = '/loan_files/customers/'.$customer_code.'/identification';
        $inputFile   = 'identification_files';
        $fileRemoved = 'identification_files_deleted';
        $fileCatch   = 'identification_files_catch';
		...
		if($this->input->post('attachment_file_deleted')){
			...
			foreach ($attachment_file_deleted as $key => $attach_id) {
				if($attach_id != null){
					$this->db->where($this->id,$attach_id)->delete($this->tblAttachmentFiles);
	...
	function updateCustomerHistory($id){
		$this->db
			 ->where($this->id, $id)
			 ->update($this->tblLoanCustomerHistory, array('deleted_by'=> $this->user->info->ID, 'deleted_at'=>date("Y-m-d H:i:s")));
			 return $this->db->affected_rows();
	}
	...
	function findSouseByCustomerID($id){
		return $this->db
					->where($this->customer_id, $id)
					->get($this->tblSpouse)
					->row();
	}
	function findContactByCustomerID($id){
		return $this->db
					->where($this->customer_id, $id)
					->order_by($this->contact_id, 'asc')
					->get($this->tblContact)
					->result();
	}
	function findIdentificationByCustomerID($id){
		return $this->db
					->where($this->customer_id, $id)
					->order_by($this->identification_id, 'asc')
					->get($this->tblIdentification)
					->result();
	}
	function findIdentificationFilesByCustomerID($id){
		return $this->db
					->where($this->customer_id, $id)
					->order_by($this->id, 'asc')
					->get($this->tblIdentificationFiles)
					->result();
	}
	function findEmploymentByCustomerID($id){
		return $this->db
					->where($this->customer_id, $id)
					->order_by($this->employment_id, 'asc')
					->get($this->tblEmploymentDetail)
					->result();
	}
	function findBusinessPlanByCustomerID($id){
		return $this->db
					->where($this->tblBusinessPlan.'.'.$this->customer_id, $id)
					->join($this->tblBusinessIncomeExpend, $this->tblBusinessPlan.'.'.$this->business_plan_id.'='.$this->tblBusinessIncomeExpend.'.'.$this->business_plan_id,'left')
					->order_by($this->tblBusinessPlan.'.'.$this->business_plan_id, 'asc')
					->get($this->tblBusinessPlan)
					->result();
	}
	function findPersonalInExByCustomerID($id){
		return $this->db
					->where($this->customer_id, $id)
					->get($this->tblPersonalIncomeExpend)
					->row();
	}
	function findAttachmentByCustomerID($id){
		return $this->db
					->where($this->customer_id, $id)
					->order_by($this->id, 'asc')
					->get($this->tblAttachmentFiles)
					->result();
	}
