...
{	
	...
	private $data = array();
	private $page = "loan/rule/";
	private $link = "loan/rule";
	private $title = "loan/rule";	
	private $moduleName = "loan/rule";	
	...
	function __construct()
	{
		...
		$this->load->model('loan/RuleModel','ruleModel');
		$this->load->helper('loan');
		$this->data['title'] = $this->title;
		$this->data['link'] = $this->link;
		$this->data['status'] = $this->ruleModel->count();
	...
	public function index(){
		if($this->authorization->hasPermission($this->moduleName, "list")){
			$this->data['data'] = $this->ruleModel->findAll();
	...
	/** 
      ...
      * @param: 21/March/2020
      * @param: method for open form create rule
      *----------------------------------------------------------------
      */
	public function add(){
	...
	/** 
      ...
      * @author: khaing.pho1991@gmail.com
      * @param: 21/March/2020
      * @param: method for submit create rule
      *----------------------------------------------------------------
      */
	...
	{	
		if($this->authorization->hasPermission($this->moduleName, "create")){
			/** 
	          ...
	          *-------------------------------
	          */
			$this->form_validation->set_rules('rule_code', 'rule Code', 'trim|required|is_unique[loan_rule.rule_code]');
			$this->form_validation->set_rules('name_en', 'Name (EN)', 'trim|required|is_unique[loan_rule.name_en]');
			$this->form_validation->set_rules('name_kh', 'Name (KH)', 'trim|required|is_unique[loan_rule.name_kh]');
	...
	/** 
      ...
      * @author: khaing.pho1991@gmail.com
      * @param: 21/March/2020
      * @param: method for open form edit rule
      *----------------------------------------------------------------
      */
	public function edit(){
		if($this->authorization->hasPermission($this->moduleName, "update")){
			if(isset($_GET['id'])){
	            if($_GET['id'] !="" && $_GET['id'] !=""){
	            		$this->data['data'] = $this->ruleModel->findOne($_GET['id']);
	...
	/** 
      ...
      * @author: khaing.pho1991@gmail.com
      * @param: 21/March/2020
      * @param: method for submit update rule
      *----------------------------------------------------------------
      */
			...
			if(isset($_GET['id'])){
	            if($_GET['id'] !="" && $_GET['id'] !="")
	            {
					/** 
			          ...
			          *-------------------------------
			          */
			        $this->form_validation->set_rules('rule_code', 'rule Type Code', 'trim|required|callback_isValidCode');
					...
					if($this->form_validation->run() == false){
						...
						/** 
				          ...
				          *-------------------------------
				          */
						$output = $this->ruleModel->update($_GET['id']);
						if($output){
	...
	function isValidCode($code){
        $isValid = $this->ruleModel->checkValidCode($_GET['id'], $code);
        if(empty($isValid)){
            $response = true;
        }else{
        	$this->form_validation->set_rules('rule_code', 'Rule Code', 'trim|required|is_unique[loan_rule.rule_code]');
    ...
    function isValidNameEN($nameEN){
        $isValid = $this->ruleModel->checkValidNameEN($_GET['id'], $nameEN);
        if(empty($isValid)){
            $response = true;
        }else{
        	$this->form_validation->set_rules('name_en', 'Name (EN)', 'trim|required|is_unique[loan_rule.name_en]');
    ...
    function isValidNameKH($nameKH){
        $isValid = $this->ruleModel->checkValidNameKH($_GET['id'], $nameKH);
        if(empty($isValid)){
            $response = true;
        }else{
        	$this->form_validation->set_rules('name_kh', 'Name (KH)', 'trim|required|is_unique[loan_rule.name_kh]');
	...
	/** 
      ...
      * @author: khaing.pho1991@gmail.com
      * @param: 21/March/2020
      * @param: method for submit delete rule
      *----------------------------------------------------------------
      */
	public function delete()
	{
		if($this->authorization->hasPermission($this->moduleName, "delete")){
			if(isset($_GET['id'])){
	            if($_GET['id'] !="" && $_GET['id'] !="")
	            {
					$output = $this->ruleModel->deleteById($_GET['id']);
					if($output){
    ...
    public function findFilter($filter = null){
      ...
      if($this->authorization->hasPermission($this->moduleName, "search")){
        if($filter != null){
            $this->data['data'] = $this->ruleModel->findAllFilter($filter);
        }else{
            $this->data['data'] = $this->ruleModel->findAll();
        }
