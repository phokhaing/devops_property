 ...
 *------------------------------------------------------
 */
class Advance_clearing extends CI_Controller
{
    /* GLOBAL VARAIBLE */
    private $data       = array();
    private $page       = "advance_clearing/";
    private $link       = "advance_clearing";
    private $title      = "advance_clearing";
    ...
    public function add()
    {
        if ($this->authorization->hasPermission($this->moduleName, "create")){
            ...
            $this->data['accounts'] = $this->advanceClearingModel->findAccountActive();
            $this->data['refno'] = $this->advanceClearingModel->generate_code();
    ...
    {
        if ($this->authorization->hasPermission($this->moduleName, "create")) {
			      if ($this->advanceClearingModel->is_validated()) 
            {
                /* validation true and submit */
                $output = $this->advanceClearingModel->create();
                    if ($output) {                       
                ...
                if (isset($_FILES[$inputFile]) && $_FILES[$inputFile]['name'][0] != '') {
                    $filePath    = '/catch_file/advance_clearing';
    ...
    public function edit()
    {
        if ($this->authorization->hasPermission($this->moduleName, "update")) {
            if (isset($_GET['id'])) {
                if ($_GET['id'] != "" && $_GET['id'] != "") {
                    $this->data['items'] = $this->advanceClearingModel->findItems($_GET['id']);
                    $this->data['data'] = $this->advanceClearingModel->findOne($_GET['id']);
                    $this->data['attachments'] = $this->advanceClearingModel->findFiles($_GET['id']); 
                    $this->data['projects'] = $this->advanceClearingModel->findProjectActive();
                    $this->data['accounts'] = $this->advanceClearingModel->findAccountActive();
                    $this->data['refno'] = $this->advanceClearingModel->generate_code();
                    /* unset session catch_files */
                ...
                if ($_GET['id'] != "" && $_GET['id'] != "") {
                    ...
                    if ($this->advanceClearingModel->is_validated()) 
                    {
                        /* validation true and update application info */
                            $output = $this->advanceClearingModel->update($_GET['id']);
                            if ($output) {
    ...
    public function delete()
    {
        if ($this->authorization->hasPermission($this->moduleName, "delete")) {
            if (isset($_GET['id'])) {
                if ($_GET['id'] != "" && $_GET['id'] != "") {
                    $output = $this->advanceClearingModel->deleteById($_GET['id']);
                    if ($output) {
                        /* update application history */
                        $this->advanceClearingModel->updateHistory($_GET['id']);
    ...
    {   
        if($this->advanceClearingModel->authorize($recordID, $status, $user))
        {         
    ...
    public function generateCode(){
        echo $this->advanceClearingModel->generate_code();
    }
}
