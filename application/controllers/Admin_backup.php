...
class Admin extends CI_Controller {
    ...
    public function announcements() {
        $this->authorization->hasAccess(strtolower('admin/announcements'));
        if($this->authorization->hasPermission(strtolower('admin/announcements'), "list")){
            // if (!$this->user->info->admin &&
            //         !$this->user->info->admin_announcements) {
            //     $this->template->error(lang("error_2"));
            // }

            $this->template->loadData("activeLink", array("admin" => array("announcements" => 1)));
    ...
    public function announcement_page() {
        ...
        foreach ($announcements->result() as $r) {
            ...
            $btn_update = null;
            $btn_delete = null;
            if($this->authorization->hasPermission(strtolower('admin/announcements'), "update")){
                $btn_update = '<a href="' . site_url("admin/edit_announcement/" . $r->ID) . '" class="btn btn-warning btn-xs" title="' . lang("ctn_55") . '"><span class="glyphicon glyphicon-cog"></span></a>';
            }
            if($this->authorization->hasPermission(strtolower('admin/announcements'), "delete")){
                $btn_delete = '<a href="' . site_url("admin/delete_announcement/" . $r->ID . "/" . $this->security->get_csrf_hash()) . '" onclick="return confirm(\'' . lang("ctn_86") . '\')" class="btn btn-danger btn-xs" title="' . lang("ctn_57") . '"><span class="glyphicon glyphicon-trash"></span></a>';
            }
    ...
    public function section_settings() {
        $this->authorization->hasAccess(strtolower('admin/section_settings'));
        if($this->authorization->hasPermission(strtolower('admin/section_settings'), "list")){
            $this->template->loadData("activeLink", array("admin" => array("section" => 1)));
    ...
    public function ticket_settings() {
        $this->authorization->hasAccess(strtolower('admin/ticket_settings'));
        if($this->authorization->hasPermission(strtolower('admin/ticket_settings'), "list")){
            $this->load->model("tickets_model");
            $this->template->loadData("activeLink", array("admin" => array("ticket_settings" => 1)));
    ...
    public function custom_fields() {
        $this->authorization->hasAccess(strtolower('admin/custom_fields'));
        if($this->authorization->hasPermission(strtolower('admin/custom_fields'), "list")){
            $this->template->loadData("activeLink", array("admin" => array("custom_fields" => 1)));
    ...
    public function user_roles() {
        $this->authorization->hasAccess(strtolower('admin/user_roles'));
        if($this->authorization->hasPermission(strtolower('admin/user_roles'), "list")){
            // if (!$this->user->info->admin)
            //     $this->template->error(lang("error_2"));
    ...
    public function add_user_role_pro() {
        if($this->authorization->hasPermission(strtolower('admin/user_roles'), "create")){
            // if (!$this->user->info->admin)
            //     $this->template->error(lang("error_2"));
    ...
    public function edit_user_role($id) {
        if($this->authorization->hasPermission(strtolower('admin/user_roles'), "update")){
            // if (!$this->user->info->admin)
            //     $this->template->error(lang("error_2"));
            $id = intval($id);
    ...
    public function edit_user_role_pro($id) {
        if($this->authorization->hasPermission(strtolower('admin/user_roles'), "update")){
            ...
            $this->admin_model->update_user_role($id, $data
            );
    ...
    public function delete_user_role($id, $hash) {
        if($this->authorization->hasPermission(strtolower('admin/user_roles'), "delete")){
            // if (!$this->user->info->admin)
            //     $this->template->error(lang("error_2"));
            if ($hash != $this->security->get_csrf_hash()) {
                $this->template->error(lang("error_6"));
    ...
    public function edit_member_pro($id) {
        if($this->authorization->hasPermission(strtolower('admin/members'), "update")){
            ...
            $manager = intval($this->input->post("manager"));
    ...
    public function social_settings() {
        $this->authorization->hasAccess(strtolower('admin/social_settings'));
        if($this->authorization->hasPermission(strtolower('admin/social_settings'), "list")){
            // if (!$this->user->info->admin && !$this->user->info->admin_settings) {
            //     $this->template->error(lang("error_2"));
            // }
            $this->template->loadData("activeLink", array("admin" => array("social_settings" => 1)));
    ...
    public function social_settings_pro() {
        ...
        //     $this->template->error(lang("error_2"));
        // }
        $disable_social_login = intval($this->input->post("disable_social_login"));
        $twitter_consumer_key = $this->common->nohtml($this->input->post("twitter_consumer_key"));
        $twitter_consumer_secret = $this->common->nohtml($this->input->post("twitter_consumer_secret"));
        $facebook_app_id = $this->common->nohtml($this->input->post("facebook_app_id"));
        $facebook_app_secret = $this->common->nohtml($this->input->post("facebook_app_secret"));
        $google_client_id = $this->common->nohtml($this->input->post("google_client_id"));
        $google_client_secret = $this->common->nohtml($this->input->post("google_client_secret"));
