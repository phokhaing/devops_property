...
{	
	...
	private $data = array();
	private $page = "supplier/";
	private $link = "supplier";
	...
	function __construct()
	{
		...
		$this->data['status'] = $this->supplierModel->count();
	...
	public function index(){
		if($this->authorization->hasPermission($this->moduleName, "list")){
			$this->data['data'] = $this->supplierModel->findAll();
	...
	public function edit(){
		if($this->authorization->hasPermission($this->moduleName, "update")){
			if(isset($_GET['id'])){
	            if($_GET['id'] !="" && $_GET['id'] !=""){
					$this->data['data'] = $this->supplierModel->findOne($_GET['id']);
	            ...
	            if($_GET['id'] !="" && $_GET['id'] !="")
	            {
					...
					if($this->form_validation->run() == false){
						...
						/** 
				          ...
				          *-------------------------------
				          */
						$output = $this->supplierModel->update($_GET['id']);
						if($output){
    ...
    function isValidName($project){
        $isValid = $this->supplierModel->checkValidName($_GET['id'], $project);
        if(empty($isValid)){
            $response = true;
        }else{
        	$this->form_validation->set_rules('name', lang('name'), 'trim|required|is_unique[supplier.name]');
	...
	public function delete()
	{
		if($this->authorization->hasPermission($this->moduleName, "delete")){
			if(isset($_GET['id'])){
	            if($_GET['id'] !="" && $_GET['id'] !="")
	            {
					$output = $this->supplierModel->deleteById($_GET['id']);
					if($output){
